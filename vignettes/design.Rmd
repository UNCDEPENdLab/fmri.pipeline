...
Level 2 FEAT outputs are stored under a top-level `feat_l2` directory. To avoid collisions across multiple Level 1 models, the default
expression includes the L1 model name in the path:

```
"{gpa$output_directory}/feat_l2/sub-{id}/ses-{session}/{l1_model}"
```

This yields paths like `feat_l2/sub-123/ses-1/basic/FEAT_LVL2_l2int.fsf` and keeps Level 2 outputs grouped by subject and L1 model.
...
- feat_l2_directory: The location of FEAT outputs for level 2 (subject-level) analyses. Defaults to a `feat_l2` folder and includes `{l1_model}` to avoid collisions across L1 models.
