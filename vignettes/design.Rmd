...
output:
  ...
  pdf_document:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{fmri.pipeline overview and design}
...
A data.frame containing variables that uniquely identify each subject and occasion, as well as person/occasion characteristics that can be included in sample-level analyses of individual differences. In cross-sectional analyses, `$subject_data` is usually one row per subject. In longitudinal analyses, it should be one row per `id`/`session`; person-level covariates such as sex, baseline age, or treatment arm should be repeated across sessions. Examples of variables that could be in `$subject_data` include mean framewise displacement for each subject/session (nuisance covariate), age, sex, personality variables, or external cognitive measures (e.g., digit span). Any covariate in this data.frame can be incorporated into level 3 (i.e., sample-level) GLM analysis. Users should not construct `$session_data` directly; it is derived internally from `$run_data` columns that are constant within `id`/`session` and from `$subject_data`.
...
`fmri.pipeline` now supports backend selection at multiple levels of specificity. The simplest workflow is still to choose one software package globally:

```r
gpa$glm_software <- "fsl"
```

This means "use FSL everywhere unless something more specific overrides it."

For more complex workflows, users can override backend choice at three additional levels:

- Per level of analysis using `gpa$level_backends`
- Per model using `execution_backend`
- At run time using arguments to `run_glm_pipeline()`
...
In other words, broad defaults are allowed, but more specific settings win.

### Global default

If you want one backend for all model setup and estimation, continue using `gpa$glm_software`:
...
If you want different software at different levels, use `gpa$level_backends`:
...
This is the most useful configuration for longitudinal `3dLMEr` analyses at present.

### Per-model overrides

If only some models should use a different backend, set a backend field on the model specification itself.

For any model level, `execution_backend` controls where that model is estimated:

```r
gpa$l3_models$models$longitudinal_main$execution_backend <- "afni"
```

For Level 3 models that consume lower-level inputs, a separate `producer_backend` field can be used to indicate which upstream backend should provide those inputs:
...
The distinction matters because the backend that estimates a model is not always the same as the backend that produces the inputs it needs. For example, an AFNI `3dLMEr` model may currently be estimated by AFNI while consuming lower-level contrast images produced by FSL.

Model-level backend fields can also be included in YAML model specifications:
...
If you want to override backend selection without changing the saved `gpa` object, pass overrides directly to `run_glm_pipeline()`:
...
This is useful when you want to try a different backend for a subset of models without permanently modifying the model specification.

### How backend resolution works in practice

The current behavior is intentionally flexible but conservative:

- If you only set `gpa$glm_software`, that backend is used as the default everywhere.
- If you also set `gpa$level_backends`, those values override the global default for the specified levels.
- If a model has `execution_backend`, that model uses that backend even if the level default is different.
- If `run_glm_pipeline()` receives `level_backends` or `backend_overrides`, those run-time values override the object.
...
- Temporary run-time backend experiments without rewriting saved model objects

### How backends declare artifacts and dependencies

Under the hood, the backend system is organized around **artifacts** and **producer requirements**. Each backend declares which artifacts it produces (e.g., `cope`, `varcope`, `dof`) and at which analysis levels it operates. When a Level 3 backend needs lower-level inputs—such as an AFNI `3dLMEr` model consuming FSL-derived COPEs—it declares the required artifacts via its specification. The pipeline resolves these requirements at submission time by matching each L3 backend's declared needs against the artifacts produced by the upstream (producer) backend. This resolution step is what makes the `producer_backend` field meaningful: it tells the pipeline which earlier backend's outputs to harvest.

The pipeline also logs a **preflight report** at submission time that shows, for every analysis level, which backends will execute, which will produce inputs for downstream levels, and whether all artifact requirements are satisfied. If you see unexpected "unsupported producer" errors, the preflight report (visible in the pipeline log at `info` level) is the first place to look.

### Current limitations

The backend-selection interface is more general than the currently implemented producer backends.

At present:

- FSL supports distinct Level 1, Level 2, and Level 3 execution
- SPM concatenates runs/sessions at Level 1 and does not run a standalone Level 2 estimation step
- AFNI `3dLMEr` is supported as a Level 3 execution backend
- `producer_backend` for Level 3 models is currently implemented only for `fsl`

So, while you can now declare producer backends explicitly, only FSL is presently supported as the upstream producer for Level 3 input harvesting. If a different producer backend is requested for Level 3 inputs, `fmri.pipeline` will stop with an explicit error rather than silently guessing.
...
Level 2 FEAT outputs are stored under a top-level `feat_l2` directory. To avoid collisions across multiple Level 1 models and lower-level contrasts, the default expression includes the L1 model, L1 contrast, and L2 model in the path:

```
"{gpa$output_directory}/feat_l2/sub-{id}/ses-{session}/L1m-{l1_model}/l1c-{l1_cope_label}/L2m-{l2_model}"
```

This yields paired paths like `feat_l2/sub-123/ses-1/L1m-basic/l1c-01_EV_face/L2m-runavg.fsf` and `feat_l2/sub-123/ses-1/L1m-basic/l1c-01_EV_face/L2m-runavg.gfeat`.
...
- feat_l2_directory: The location of FEAT outputs for level 2 (subject-level) analyses. Defaults to a `feat_l2` folder and includes `{l1_model}`, `{l1_cope_label}`, and `{l2_model}` to avoid collisions across model and contrast combinations.
