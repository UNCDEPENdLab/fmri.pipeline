# This function generates syntax for FSL Feat .fsf files for the EVs tab of a first-level fMRI analysis. It accepts a numeric design matrix whose colum names correspond to individual EVs in the model
...
- regressors:
  ...
  A list of regressors containing the specification for each one as an
  EV.

## Value

A character vector containing .fsf syntax for the EV portion of a
first-level Feat analysis based on `regressors`.

## Details

Each element of the `regressors` list is itself a list specifying how to
handle a given regressor. Required elements of the list are:
