...
- gpa:
  ...
  the \`glm_pipeline_arguments\` object with relevant fields already
  populated, including \`\$l1_models\`, \`\$l2_models\`, and/or
  \`\$l3_models\`.
...
This function exports the configuration of level 1, 2, and 3 GLM models
in a \`glm_pipeline_arguments\` (aka 'gpa') object to a YAML or JSON
file. This allows the user to setup their models from this configuration
file, rather than having to go through the building process manually
(with prompts).

Typically, the user will first run \`build_l1_models\`,
\`build_l2_models\` (for multi-run data), and \`build_l3_models\`, then
run \`export_glm_config\` so that the decisions made during the build
process are codified in the resulting file.
