...
\`path_sanitize()\` removes the following: - \[Control
...
Unix reserved filenames (\`.\` and \`..\`) - Trailing periods and spaces
(invalid on Windows) - Windows reserved filenames (\`CON\`, \`PRN\`,
\`AUX\`, \`NUL\`, \`COM1\`, \`COM2\`, \`COM3\`, COM4, \`COM5\`,
\`COM6\`, \`COM7\`, \`COM8\`, \`COM9\`, \`LPT1\`, \`LPT2\`, \`LPT3\`,
\`LPT4\`, \`LPT5\`, \`LPT6\`, LPT7, \`LPT8\`, and \`LPT9\`) The
...
Adapted slightly from fs package. Added parentheses as illegal
characters given FSL evaluates these in unquoted filenames
...
path_sanitize(str)
#> Error in path_sanitize(str): could not find function "path_sanitize"

path_sanitize("..")
#> Error in path_sanitize(".."): could not find function "path_sanitize"
```
