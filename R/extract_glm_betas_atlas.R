  ...
  for (l1 in 1:n_l1_copes) {
    ...
    l2_loop_outputs <- foreach(l2=iter(1:n_l2_contrasts), .packages=c("oro.nifti", "dplyr")) %dopar% {
      ...
      for (ai in 1:length(atlas_imgs)) {
        ...
        coef_df <- coef_df %>% 
          inner_join(l1_subinfo %>% select(numid, id, fsldir), by="numid") %>%
...
#use this for 1st-level results
# l1_inputs <- feat_l2_inputs_df$feat_dir
# l1_inputs <- sub("/gpfs/group/mnh5174/default", "/Users/michael/longleaf/studies", l1_inputs)
# 
# xx <- read_feat_inputs(l1_inputs)
...
#     message("Creating output directory: ", model_output_dir)
...
#       #reshape into data.frame with beta, numeric numid, and vnum
#       if (extract_z) {
#         coef_df <- reshape2::melt(coef_mat, varnames=c("numid", "vnum"), value.name="zstat")
#       } else {
#         coef_df <- reshape2::melt(coef_mat, varnames=c("numid", "vnum"), value.name="beta")
#       }
#       coef_df <- coef_df %>% inner_join(a_coordinates, by="vnum") %>% select(-i, -j, -k) %>% inner_join(l1_subinfo %>% select(numid, id, fsldir), by="numid")
#       
#       atlas_df[[ai]] <- coef_df
#     }
#     
