...
interpolate_decon <- function(windowed_ts, time_before=-3, time_after=3, output_resolution=1.0, aggregate=TRUE) {
  interpolated_result <- windowed_ts %>%
    group_by(atlas_value, 
  
  interpolated_result <- lapply(windowed_ts, function(byval) {
    to_interpolate <- byval %>% 
