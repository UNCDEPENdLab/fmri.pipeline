...
add_base_logger_appenders <- function(
  ...
  if (isTRUE(gpa$log_json) && !is.null(log_json_path) && nzchar(log_json_path)) {
    if (!json_appender_name %in% names(lg$appenders)) {
      lg$add_appender(lgr::AppenderJson$new(log_json_path), name = json_appender_name)
    }
  }

  if (isTRUE(gpa$log_txt) && !is.null(log_txt_path) && nzchar(log_txt_path)) {
    if (!txt_appender_name %in% names(lg$appenders)) {
      lg$add_appender(lgr::AppenderFile$new(log_txt_path), name = txt_appender_name)
    }
  }
