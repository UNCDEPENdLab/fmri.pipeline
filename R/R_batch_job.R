...
R_batch_job <- R6::R6Class("batch_job",
  private = list(
    ...
    # sequence id for use with R_batch_sequence and job tracking
    seq_id = NULL,
    ...
    gather_tracking_args = function() {
      ...
      tracking_args <- list(job_name = self$job_name, sequence_id = self$sequence_id, 
  ...
  active = list(
    #' @field sequence_id Used to identify sequence when using R_batch_sequence
    sequence_id = function(val) {
      if (missing(val)) {
        return(private$seq_id)
      } else {
        if(checkmate::test_string(val)) {
          private$seq_id <- val
        } else if (checkmate::test_integerish(val)) {
          private$seq_id <- as.character(val)
        } else {
          warning("Sequence ID must be a character string or an integer")
          return(invisible(NULL))
        }
      }
    }
  ...
  public = list(
    ...
    initialize = function(batch_directory = NULL, parent_jobs = NULL, job_name = NULL, n_nodes = NULL, n_cpus = NULL,
                          wall_time = NULL, mem_per_cpu = NULL, mem_total = NULL, r_code = NULL, r_script = NULL,
...
R_batch_sequence <- R6::R6Class("batch_sequence",
  private = list(
    ...
    # function to check if jobs already have sequence id
    check_sequence_ids = function() {
      all_null = T
      job_list <- private$sequence_jobs
      njobs <- length(job_list)
      for (ii in seq_len(njobs)) {
        if(!is.null(job_list[[ii]]$sequence_id)) {
          all_null = F
        }
      }
      if(isFALSE(all_null)) {
        warning("Some jobs in this sequence have already been assigned sequence ids.\nSubmitting this sequence will override them!",
                call. = F)
      }
      return(invisible(all_null))
    }
  ),
  public = list(
    #' @field sequence_id Optional identifier for all jobs in this sequence, used for job tracking
    sequence_id = NULL,
    ...
    #' @param sequence_id Optional identifier for all jobs in this sequence;
    #'                      used for job tracking
    initialize = function(..., joblist = NULL, sequence_id = NULL) {
    ...
    submit = function() {
      ...
      for (ii in seq_len(njobs)) {
        ...
        this_job$sequence_id <- self$sequence_id
        this_job$submit()
        if (ii < njobs) {
