test_that("lookup_* functions work correctly on a valid 4D NIfTI file", {
  ...
  expect_equal(fmri.pipeline:::lookup_run_volumes(tmpfile), dims[4])
  
  # Test lookup_nvoxels
  expect_equal(fmri.pipeline:::lookup_nvoxels(tmpfile), prod(dims))
  
  # Test lookup_dim
  expected_dims <- c(dim_x = dims[1], dim_y = dims[2], dim_z = dims[3], dim_t = dims[4])
  expect_equal(fmri.pipeline:::lookup_dim(tmpfile), expected_dims)
})
