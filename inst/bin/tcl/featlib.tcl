...
for { set i 1 } { $i <=  $fmri(multiple) } { incr i 1 } {
	#for randomise
	set switchgrouplim [expr {round($fmri(multiple)/2)}]
    if { $fmri(mixed_yn) == 4 } {
		set fmri(groupmem.${i}) $i
		if {$i > $switchgrouplim} {
			set fmri(groupmem.${i}) [expr {$i - $switchgrouplim}]
		}
    } else {
		# not randomise
		set fmri(groupmem.${i}) 1
...
LabelSpinBox $fmri(unwarpff).opts1.dwell -label "Effective EPI echo spacing (ms) " -textvariable fmri(dwell) -range { 0.0 200000 0.01 } -width 5
LabelSpinBox $fmri(unwarpff).opts1.te -label "  EPI TE (ms) " -textvariable fmri(te) -range {0.0 200000 0.01 } -width 5
...
foreach rawstats $rawstatslist {
    ...
    if { $firsttime == 1 } {
	set nonparametrically ""
	if { $fmri(mixed_yn)==4 } {
	    set nonparametrically " nonparametrically"
	}
	set ps "$ps Z (Gaussianised T/F) statistic images were thresholded$nonparametrically"
    }
